...
if(isset($_POST['submit'])){
        ...
        $sql = "select * from customer where email = '".$email."' and password = '".$pass."'" or die("failed");
        $sql1 = "INSERT INTO customer (username, email, password) VALUES('$_POST[username]', '$_POST[email]', '$_POST[password]')";
        
        $select_users = mysqli_query($conn,$sql);

        if(mysqli_num_rows($select_users) > 0)
        {
            echo '<p style="font-size: 20px;position: absolute;top: 85%;left: 35%;">User already exists...!</p>';
        }
        else
        {
            if($_POST['cpassword'] != $_POST['password'])
            {
                echo '<p style="font-size: 20px;position: absolute;top: 85%;left: 35%;">password and confirm password does not match...!</p>';
            }
            else
            {
